<?php 
	...
	$user_data=mysqli_fetch_assoc($select_user1);
		// echo "<pre>"; print_r($select_user1); exit;

	$data = '';

	if ($select_user1->num_rows < 0) {
		$data['status'] = '1';
		header("Location: http://localhost/pharma-master/login.php?status=".$data['status']);
	}else{
		$_SESSION['user_id'] = $user_data['id'];
		$_SESSION['user_email'] = $user_data['email'];
		$_SESSION['name'] = $user_data['name'];
		header('Location: http://localhost/pharma-master/');
	}
